...
* 그리고 마지막으로 구체성의 값이 다릅니다. (이에 대한 자세한 내용은 이후 [구체성](#49-구체성) 수업 참고)
...
## 4.9 구체성
**구체성**
* 선택자에는 어떤 규칙이 우선으로 적용되어야 하는지에 대해 정해진 규칙이 있는데, 이를 '구체성'이라고 함.
* 구체성은 선택자를 얼마나 명시적으로(구체적으로) 선언했느냐를 수치화한 것으로, 구체성의 값이 클수록 우선으로 적용이 된다.
```
0, 0, 0, 0
```
...
```
* 첫 번째는 0, 1, 0, 1의 구체성을 가지는 선택자로 스타일을 선언했고, 두 번째는 요소에 직접 인라인 스타일 방식으로 스타일을 선언했다.
...
* 상속되는 속성들은 보통 상식적으로 구분될만한 속성들이며, 후에 속성들에 대해 배우게 되면 자연스럽게 이해할 수 있음
...
<!-- <em>에는 color:red;가 적용되는데, 그 이유는 상속된 속성은 아무런 구체성을 가지지 못하기 때문임 -->
<!-- 0,0,0,0 도 갖지 않아서 전체 선택자의 구체성에 밀린다 -->
```

## 4.11 캐스케이딩
**cascading 규칙**
1. 중요도(!important)와 출처(출처 =  CSS 출처)
2. 구체성
3. 선언 순서
...
* 제작자의 경우는 사이트를 실제 제작하는 개발자가 작성한 CSS를 의미
* 사용자의 경우는 웹 페이지를 방문하는 일반 사용자들이 작성한 CSS를 의미 (사용자 CSS는 이젠 거의 사용되지 않는다)
* 사용자 에이전트의 경우는 일반 사용자의 환경, 즉 브라우저에 내장된 CSS를 의미

**CSS 출처에 따라 스타일 적용되는 방식**
